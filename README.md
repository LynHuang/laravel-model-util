...
- **请求过滤器 QueryFilter**：请求参数自动映射为 Eloquent 查询条件，支持操作符、`&&` / `||` 组合、排序、分页、时间范围、随机取 N 条及关联表过滤。
- **批量操作 BatchHelper**：批量更新（`CASE WHEN`）、批量插入、批量插入或更新（upsert）、批量删除、批量软删除 / 恢复。
- **模型层工具**：树形结构、排序重排、多语言字段、字段加解密 / 脱敏、操作审计、状态机、唯一编号生成。
- **通用辅助**：统一 API 响应、防重复提交（幂等）、经纬度距离与附近查询、SQL 日志。
...
**1. 新建过滤器类**（方法名即请求参数名，可隐藏数据库真实字段名）：
...
| [请求过滤器 QueryFilter](docs/query-filter.md) | 操作符速查表、组合条件、排序分页随机、关联过滤、动态参数、异常处理 |
| [批量操作 BatchHelper](docs/batch-helper.md) | 批量更新 / 插入 / upsert / 删除 / 软删除恢复、参数说明 |
| [模型层工具](docs/model-tools.md) | 树形结构、排序重排、多语言、字段加解密 / 脱敏、操作审计、状态机、唯一编号 |
